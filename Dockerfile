...
# Install dependencies
    apk add --no-cache freetype libpng libjpeg-turbo freetype-dev libpng-dev libjpeg-turbo-dev git unzip mailcap \
        bash python make gcc g++ libstdc++ linux-headers binutils-gold \
# Install node & npm
    && mkdir -p /usr/local/nvm \
    && curl -s https://raw.githubusercontent.com/creationix/nvm/v0.33.11/install.sh | NVM_DIR=/usr/local/nvm NVM_METHOD=script bash \
...
# cleanup to reduce the already large image size
    && apk del --no-cache freetype-dev libpng-dev libjpeg-turbo-dev git unzip bash python make gcc g++ linux-headers binutils-gold \
    && rm -rf /bin/.cache \
